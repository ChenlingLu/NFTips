package io.nftips.kunpeng.common;

/**
 * @author zhoujunwen
 * @date 2022-10-2022/10/21 上午9:39
 **/
public class MappingPath {
    /**
     * 通过图像查询
     */
    public static final String IMAGE_SEARCH_V1 = "query/image/v1";
    public static final String CATEGORY_ID_SEARCH_V1 = "query/categoryid/v1";
    public static final String CATEGORY_LIST_SEARCH_V1 = "query/category/list/v1";

    public static final String TOP10_SEARCH_V1 = "query/top10/v1";

    /**
     * 数据分析
     */
    public static final String Data_Analy_V1 = "query/data/analy/v1";
}
